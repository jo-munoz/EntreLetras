...
namespace JuegoDidactico.Web.WebForms.Modules.Children.Math.Level2
{
    public partial class frmMathLevel2 : System.Web.UI.Page
    {
        ...
        protected void btnLevel2_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (Convert.ToInt32(txt1_Resul.Text) == 258 && Convert.ToInt32(txt2_Resul.Text) == 96)
                    {
                        inPuntaje = 100;
                    }
                    else if (Convert.ToInt32(txt1_Resul.Text) == 258)
                    {
                        inPuntaje = 50;
                    }
                    else if (Convert.ToInt32(txt2_Resul.Text) == 96)
                    {
                        inPuntaje = 50;
                    }
                    else
                    ...
                    {
                        //Inserto el Avance
                        stLevelInsert = csLevel.stLevelInsert(Session["Cod_user"].ToString().Trim(), "Level2", Convert.ToString(inPuntaje).Trim(), "Math");
                        if (stLevelInsert == "Se inserto con exito")
                        {
                            Session["LevelPassedMath"] = "Level2";
                            Response.Redirect("~/WebForms/Modules/Children/Math/frmSelectLevel.aspx", false);
                        }
                    }
                }
